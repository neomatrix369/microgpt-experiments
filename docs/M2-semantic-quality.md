...
- **Checked-in example artifacts**: **`example-experiments/`** holds two full **`output_*.txt`** run reports, a saved **`compare_run_reports.py`** transcript, and **`comparison_report.html`** for the **H4 vs H1** @ **1000-step** pair (same setup as *Run experiments examples*). See README *Example artifacts (preview)*. Local sweeps still use gitignored **`outputs/`**.
