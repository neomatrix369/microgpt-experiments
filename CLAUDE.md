...
Run **`pytest`** from the repo root: `python -m pytest tests/ -q` (evaluation, quality hub, experiment API, sweep grid, **run timing**, paths, text loss plots, HTML report generator). See `docs/M2-semantic-quality.md` for the semantic-quality slice log.
