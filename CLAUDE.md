...
| `microgpt_updated.py` | Refactored variant: `from __future__ import annotations`, type hints, `Tokeniser` / `StateDict` / `KVCache`, `train()` / `generate()` / `main()`, `save_run_report()`, and richer comments. Prefer this for changes that need structure or tests. |
...
- **Autograd**: `Value` implements forward ops with local gradients; `loss.backward()` walks the graph. No PyTorch.
- **Model**: GPT-2–like stack with **RMSNorm** (not LayerNorm), **no biases**, **ReLU** in the MLP (not GELU). KV caches for attention are part of the live graph during training (not detached).
...
- Prefer **`microgpt_updated.py`** for readability and for edits that need clear function boundaries; use **`microgpt.py`** when the user wants a minimal diff against the “canonical” one-file narrative.
