...
def run_parameter_glossary_lines() -> list[str]:
    ...
    return [
        "--- Parameter glossary ---",
        "Units: layers = transformer blocks; dimensions = vector width;",
        "  heads = attention head count; tokens = context positions;",
        "  steps = optimizer updates; dimensionless = no physical unit.",
        "Architecture / data:",
        "  N_LAYER   — Transformer depth (stacked attention+MLP blocks). Unit: layers.",
        "  N_EMBD    — Hidden width; each token position is a vector this long. Unit: dimensions.",
        "  N_HEAD    — Number of attention heads; chosen with N_EMBD (per-head width is N_EMBD//N_HEAD). Unit: heads.",
        "  HEAD_DIM  — Calculated from N_EMBD and N_HEAD (N_EMBD // N_HEAD); stored as HEAD_DIM=;",
        "            comparison tools annotate it as a derived field, not a separate sweep knob. Unit: dimensions.",
        "  BLOCK_SIZE — Max context length (positions 0..BLOCK_SIZE-1); must fit longest line + BOS. Unit: tokens.",
        "Training / optimisation:",
        "  NUM_STEPS — One training step = one document forward-backward + Adam update. Unit: steps.",
        "  LEARNING_RATE — Base Adam step size (scaled by linear decay to 0 over the run). Unit: dimensionless.",
        "  BETA1, BETA2 — Adam momentum and variance decay. Unit: dimensionless.",
        "  EPS_ADAM — Numerical stability in Adam denominator. Unit: dimensionless.",
        "  SEED — RNG seed (init + shuffling + sampling) for reproducibility. Unit: dimensionless (integer).",
        "Inference:",
        "  TEMPERATURE — Logits divided by this before softmax; lower = sharper samples. Unit: dimensionless.",
        "Data:",
        "  INPUT_PATH — Training text file (one document per line; BOS wraps each line). Unit: path.",
        "",
        "Run timing (--- Run timing --- block; authoritative wall-clock bounds):",
        "  RUN_STARTED_UTC / RUN_ENDED_UTC — ISO-8601 instants in UTC.",
        "  RUN_STARTED_LOCAL / RUN_ENDED_LOCAL — same instants in local time with offset.",
        "  DURATION_SECONDS — elapsed wall time from start of training through evaluation.",
        "  TIMEZONE — local timezone name at run end (for human context).",
        "  During training, stdout also shows live ``elapsed … | ETA …`` per step; sweeps show sweep-level progress too.",
        "",
        ...
        "  YYYYMMDD_HHMMSS = local wall-clock suffix when the report path is built (approximate end).",
        "  Authoritative start/end/duration live in --- Run timing --- (UTC + local with offset).",
